...
const inserirVenda = async (req, res) => {
  try {
    ...
    return res.status(201).json({ mensagem: 'Venda inserida com sucesso' });
  } catch (error) {
    console.error('Erro ao inserir venda:', error.message);

    return res
      .status(500)
      .json({ erro: 'Erro ao inserir venda no banco de dados' });
  }
};
...
const atualizarVenda = async (req, res) => {
  try {
    ...
    return res
      .status(500)
      .json({ erro: 'Erro ao atualizar venda no banco de dados' });
  }
};
