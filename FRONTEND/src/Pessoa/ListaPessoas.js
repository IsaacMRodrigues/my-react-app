...
const TablePessoas = ({ pessoas, pessoaParaAtualizar }) => {
  ...
  const filteredPessoas = pessoas.filter((pessoa) => {
    const pagoMatch = (mostrarTodos || pessoa.pago === 0) && pessoa.nomePessoa.toLowerCase().includes(searchTerm.toLowerCase());
    return pagoMatch;
  });
        ...
        <TableBody>
          {filteredPessoas.map((pessoa) => (
            ...
            >
              ...
              <TableCell align="center">{pessoa.telefonePessoa}</TableCell>
              <TableCell align="center">{new Date(pessoa.dataCompra).toLocaleDateString()}</TableCell>
              <TableCell align="center">R$ {pessoa.valorCompra}</TableCell>
              <TableCell align="center">{pessoa.descricaoCompra}</TableCell>
              <TableCell style={{ color: pessoa.pago === 0 ? "red" : "green" }} align="center">{pessoa.pago === 0
                  ? "PAGAMENTO PENDENTE"
                  : "PAGAMENTO CONCLUÍDO"}</TableCell>
                  {pessoa.pago === 0 ? (
                <TableCell align="center">
                  <Button variant="outlined" onClick={() => definirPago(pessoa)}>
                    Definir como pago
                  </Button>
                </TableCell>
              ) : (
                <TableCell  align="center">{new Date(pessoa.dataPagamento).toLocaleDateString()}</TableCell>
              )}
            </TableRow>
          ))}
        </TableBody>
